...
\section{制御モードの分類}
%- I型: 線速度一定→理論的に不可能→不可
%- II型: 厳密な差分方程式→誤差がひどい→不可
%- II-B型: pi'=pi * v/Fで割り込み時間を補正(Leib Rampで計算した割り込み時間を補正する)→やってみる
% v:極座標の線速度 v = sqrt(drdt-r^2 )
% F:追従すべき線速度F (7月末)
%- III型: エクストルーダ側をrに従って動かす
%- IV型: ステージ側をrに従って動かす
%- III+IV型：IIIとIVのハイブリッド。(7月末)
%  主にIVで動くが、原点付近で速度が発散するので、
%　速度は打ち止めて、III型に切り替える
%- V型: x-y座標を新しく作る
%
%VI型：rだけでなく、⊿rを考慮してスクリュー制御
%　→関係ないのかな？
%　→θではなく、rが支配的な場合の「エクストルーダ」制御
%　→改良III型
%VII型：rだけでなく、⊿rを考慮してステージ制御
%　→θではなく、rが支配的な場合の「ステージ」制御
%　→改良IV型
