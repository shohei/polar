...
スライサーが吐き出したG-codeに記述されたエクストルーダの値から，スクリューの速度プロファイルを逆算する．いま，(\ref{eq:v})式に従って$v$が推移するとき，$\dot{r}$と$\dot{\theta}$は一意に決定できない．ここで独立変数を減らすために、$\theta$は今までと同じように台形型の加減速プロファイル（とくに高速計算アルゴリズム{\bf leib ramp}：詳しくは文献\cite{leib}をみよ）をとると仮定する。
