...
実際に実験を行ったところ改善が見られたので、以降はこの方法を採用する．なぜこの方法がすぐれているのか，その原因については後述する．
...
スライサーが吐き出したG-codeに記述されたエクストルーダの値から，スクリューの速度プロファイルを逆算する．いま，(\ref{eq:v})式に従って$v$が推移するとき，$\dot{r}$と$\dot{\theta}$は一意に決定できない．ここで独立変数を減らすために，$\theta$は今までと同じように台形型の加減速プロファイル（とくにleib ramp：詳しくは文献\cite{leib}を参照）をとると仮定する．leib rampは乗算器と加算器のみでステッピングモーターのパルス遅延時間を高速に計算するアルゴリズムであり\footnote{台形の立ち上がり部分を3次関数で表現している．}，Marlinでも実装されている．
\clearpage
...
\end{algorithm}
\vspace{-0.7cm}
\captionof{figure}{Leib Ramp}
\vspace{0.3cm}

このとき，$\dot r$は以下のように逆算できる。
...
\end{algorithm}
\vspace{-0.7cm}
\captionof{figure}{$\dot r$ solver algorithm}
\vspace{0.3cm}
...
上で求めた速度プロファイルを用いて加減速制御を用いたときに，発生する出力物の誤差を$e_{ideal}$と置けば，$e_{ideal}$は以下のように表される．
\begin{eqnarray*}
	 | e_{ideal} | &=& \int f(t) dt
\end{eqnarray*}
