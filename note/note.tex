...
$\alpha = 6.0$、フィルタ段数を39、通過域端周波数$\omega_p = 0.005 \pi$、阻止域端周波数$\omega_s = 0.01 \pi$として設計したFIRフィルタのインパルス応答と正規化振幅特性$H(e^{j \omega})$をFigure \ref{eq:fir_window}に示す。

\begin{figure}[htbp]
    \includegraphics[bb=0 0 432 216,width=1\columnwidth]{impulse_h.pdf}
    \caption{FIR filter with kaiser window function}
   \label{eq:fir_window}
\end{figure}
...
フィルタリング後のSNW時系列データをFigure \ref{fig:filtered}に示す。
\begin{figure}[htbp]
