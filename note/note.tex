...
スライサーが吐き出したG-codeに記述されたエクストルーダの値から，スクリューの速度プロファイルを逆算する．いま，(\ref{eq:v})式に従って$v$が推移するとき，$\dot{r}$と$\dot{\theta}$は一意に決定できない．ここで独立変数を減らすために，$\theta$は今までと同じように台形型の加減速プロファイル（とくにleib ramp：詳しくは文献\cite{leib}を参照）をとると仮定する．leib rampは乗算器と加算器のみでステッピングモーターのパルス遅延時間を高速に計算するアルゴリズムであり，Marlinでも実装されている．


このとき，$\dot r$を逆算したものを図に示す．
