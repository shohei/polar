...
\section{予想される問題点}
\subsection{半径の近似精度} 
(4)式に表される$r_i$の計算において，$r_i$と$r_{i-1}$の平均値を用いているが，$r$が急激に増減する場合にこの部分の誤差の影響が大きくなる．例えばヒトデ型や螺旋形状を高速に造形する際に問題が懸念されるが，これは与えられるG-codeの半径方向の刻み($ = \Delta r$)が小さい場合，実際は問題にならないと考えられる．
\subsection{時間の刻み}
(4)式の導出の際に$\Delta t$が一定であると仮定してくくりだしている．現実装では$\Delta t$がG-codeの行ごとの経過時間がほぼ一定である（差の絶対値が小さい）ことを仮定しているが\footnote{G-codeが到着したタイミングで移動変位$\Delta r, \Delta \theta$を計算している．}，これはスライサに依存している．したがって精度が悪い場合は再検討が必要である．
